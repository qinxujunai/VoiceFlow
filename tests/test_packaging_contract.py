...
def test_windows_executable_has_product_version_metadata():
    ...
    assert "filevers=(0, 3, 1, 3)" in version
    assert "prodvers=(0, 3, 1, 3)" in version
    assert "StringStruct('FileVersion', '0.3.1.3')" in version
    assert "StringStruct('ProductVersion', '0.3.1+260811.3')" in version
...
def test_release_candidate_has_a_traceable_build_id_everywhere():
    ...
    assert 'BUILD_ID = "260811.3"' in application
    assert "display_version()" in overlay
    assert '#define MyAppBuildId "260811.3"' in installer
    assert "VersionInfoVersion=0.3.1.3" in installer
    assert "build 260811.3" in notes
